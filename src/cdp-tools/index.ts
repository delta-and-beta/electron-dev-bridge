...
// ---------------------------------------------------------------------------
// getCdpTools — the single public export
// ---------------------------------------------------------------------------
...
export function getCdpTools(
  ...
  // -----------------------------------------------------------------------
  // Shared helper — get bounding box for a CSS selector
  // -----------------------------------------------------------------------

  async function getBoundingBox(selector: string) {
  ...
  tools.push({
    ...
    handler: async ({ selector }: { selector: string }) => {
      ...
      const { attributes: attrArray } = await client.DOM.getAttributes({
        nodeId,
      })
  ...
  tools.push({
    ...
    handler: async ({ selector }: { selector: string }) => {
      ...
      const elements: Array<{ nodeId: number; outerHTMLPreview: string }> = []
  ...
  // 7. electron_get_accessibility_tree
  tools.push({
    definition: {
  ...
  tools.push({
    ...
    handler: async ({ selector }: { selector: string }) => {
      ...
      const text = await bridge.evaluate(`
        (() => {
          const el = document.querySelector(${JSON.stringify(selector)});
  ...
  tools.push({
    ...
    handler: async ({ direction = 'down', amount = 500, selector }: { direction?: string; amount?: number; selector?: string } = {}) => {
      bridge.ensureConnected()
  ...
  tools.push({
    ...
    handler: async ({ selector, fullPage = true }: { selector?: string; fullPage?: boolean } = {}) => {
      ...
      screenshotCounter++
      const filename = `screenshot-${Date.now()}-${screenshotCounter}.${screenshotFormat}`
  ...
  // 21. electron_compare_screenshots
  tools.push({
    definition: {
